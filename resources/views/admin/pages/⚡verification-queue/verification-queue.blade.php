<div class="p-8">
    ...
    {{-- Flash message --}}
    @if (session('status'))
        <div class="mb-6 flex items-start gap-2.5 px-5 py-3 bg-[#123524]/5 dark:bg-emerald-500/10 border border-[#123524]/10 dark:border-emerald-500/20 rounded-xl text-[#123524] dark:text-emerald-400 text-sm font-medium">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-2xl bg-[#D4A537]/15 dark:bg-[#D4A537]/20 flex items-center justify-center text-[#a97f1f] dark:text-[#E5B94A] shrink-0">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
            </div>
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-[#123524] dark:text-white" style="font-family: 'Fraunces', serif;">
                    Alumni Verification Queue
                </h1>
                <p class="text-[#123524]/60 dark:text-white/60 mt-0.5 text-sm">Board-program alumni awaiting manual verification of their PRC board exam results.</p>
            </div>
        </div>
    ...
    <div class="space-y-4">
        @forelse ($pendingUsers as $user)
            ...
            <div wire:key="user-{{ $user->id }}" class="bg-white dark:bg-[#242526] rounded-2xl p-6 shadow-sm border border-[#123524]/5 dark:border-white/5 hover:shadow-md transition-shadow flex flex-col md:flex-row md:items-start justify-between gap-6">
                ...
                <div class="flex-1 flex items-start gap-4">
                    ...
                    @if ($avatarUrl)
                        ...
                        <div style="display: none;" class="w-11 h-11 rounded-full bg-[#123524]/10 dark:bg-[#D4A537]/15 items-center justify-center text-[#123524] dark:text-[#D4A537] font-bold shrink-0">
                            {{ $initial }}
                        </div>
                    @else
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="flex flex-wrap items-center gap-2 mb-1">
                            <h3 class="font-bold text-[#123524] dark:text-white">{{ $user->name }}</h3>
                        ...
                        <p class="text-sm text-[#123524]/60 dark:text-white/60">{{ $user->email }}</p>
                        ...
                        {{-- Rejection banner --}}
                        @if ($user->rejected_at)
                            <div class="mt-3 text-sm text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 rounded-xl p-3.5">
                                <div class="text-xs text-red-500 dark:text-red-400 font-semibold tracking-wide uppercase flex items-center gap-1.5">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                ...
                                <div class="text-xs text-red-400 dark:text-red-400/70 mt-1">{{ $user->rejected_at->diffForHumans() }}</div>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="flex md:flex-col gap-2 shrink-0 md:w-36">
                    <button wire:click="approve({{ $user->id }})"
                        ...
                        class="flex-1 md:flex-none inline-flex items-center justify-center gap-1.5 px-5 py-2.5 bg-[#123524] dark:bg-[#D4A537] text-white dark:text-[#123524] text-sm font-semibold rounded-xl hover:bg-[#0d2819] dark:hover:bg-[#E5B94A] transition whitespace-nowrap">
                    ...
                    <button wire:click="openRejectModal({{ $user->id }})"
                        class="flex-1 md:flex-none inline-flex items-center justify-center gap-1.5 px-5 py-2.5 bg-white dark:bg-[#3A3B3C] border border-red-300 dark:border-red-500/40 text-red-600 dark:text-red-400 text-sm font-semibold rounded-xl hover:bg-red-50 dark:hover:bg-red-500/10 transition whitespace-nowrap">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        @empty
            <div class="text-center py-16 bg-white dark:bg-[#242526] rounded-2xl border border-[#123524]/5 dark:border-white/5">
                ...
                <p class="text-[#123524]/50 dark:text-white/50">No pending board exam verifications right now.</p>
            </div>
        @endforelse
    </div>
    ...
    <div class="mt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <p class="text-sm text-[#123524]/60 dark:text-white/60 text-center sm:text-left">
            Showing
            <span class="font-semibold text-[#123524] dark:text-white">{{ $pendingUsers->count() ?? 0 }}</span>
            of
            <span class="font-semibold text-[#123524] dark:text-white">{{ $pendingUsers->total() }}</span>
        ...
        <div class="grid grid-cols-2 sm:inline-flex gap-x-2">
            ...
            @else
                <button wire:click="previousPage" wire:loading.attr="disabled" wire:target="previousPage,search"
                    class="px-4 py-2 inline-flex items-center justify-center gap-x-1 text-sm font-semibold rounded-lg bg-[#123524] dark:bg-[#D4A537] text-white dark:text-[#123524] hover:bg-[#0d2819] dark:hover:bg-[#E5B94A] transition disabled:opacity-50">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2"
                        stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
                        <path d="M12 15l-6-6 6-6" />
                    </svg>
    ...
    {{-- Reject reason modal --}}
    @if ($rejectingUserId)
        <div class="fixed inset-0 bg-black/40 dark:bg-black/60 flex items-center justify-center z-50 px-4"
             wire:click.self="closeRejectModal">
            <div class="bg-white dark:bg-[#242526] rounded-2xl p-6 w-full max-w-sm shadow-2xl dark:border dark:border-white/5">
                ...
                <h3 class="text-lg font-bold text-[#123524] dark:text-white" style="font-family: 'Fraunces', serif;">
                    Reject Application
                </h3>
                <p class="text-sm text-[#123524]/60 dark:text-white/60 mt-1 mb-4">
                ...
                <textarea wire:model="rejectReasonInput" rows="3" placeholder="e.g. Board exam date does not match PRC records."
                    class="w-full px-4 py-3 rounded-xl border border-[#123524]/15 dark:border-white/10 bg-white dark:bg-[#3A3B3C] text-sm text-[#123524] dark:text-white placeholder-[#123524]/30 dark:placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-[#D4A537] focus:border-transparent transition"></textarea>

                <div class="flex gap-3 mt-5">
                    <button wire:click="closeRejectModal"
                        class="flex-1 px-4 py-2.5 rounded-xl border border-[#123524]/15 dark:border-white/10 text-[#123524] dark:text-white text-sm font-semibold hover:bg-[#123524]/5 dark:hover:bg-white/5 transition">
                        Cancel
                    </button>
