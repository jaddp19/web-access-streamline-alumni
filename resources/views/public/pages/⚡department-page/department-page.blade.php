            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                @forelse ($this->departments as $department)
                    @php
                        ...
                        if ($department['dept_logo']) {
                            $logoUrl = $department['dept_logo'] === 'CSAV-LOGO'
                                ? 'https://tse2.mm.bing.net/th/id/OIP.D0DJ0ePPxNcvYOeq6q9esQAAAA?pid=Api&P=0&h=180'
                                : (filter_var($department['dept_logo'], FILTER_VALIDATE_URL)
                                    ? $department['dept_logo']
                                    : \Illuminate\Support\Facades\Storage::url($department['dept_logo']));
                        }
                    @endphp
                    ...
                    <div wire:key="dept-{{ $department['id'] }}"
                        ...
                        {{-- Name --}}
                        <h3 class="text-base sm:text-lg font-bold text-[#123524]" style="font-family: 'Fraunces', serif;">
                            ...
                            <div class="mt-4 pt-4 border-t border-[#123524]/5">
                                ...
                                <ul class="space-y-1.5">
                                    @foreach ($department['courses'] as $course)
                                        <li class="flex items-start gap-2 text-xs sm:text-sm text-[#123524]/70">
                                            <svg class="w-3.5 h-3.5 mt-0.5 shrink-0 text-[#D4A537]" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                ...
                @empty
                    <div class="col-span-full text-center py-12 sm:py-16">
                        ...
                        <p class="text-[#123524]/60 text-sm sm:text-base">No departments available yet.</p>
                    </div>
                @endforelse
            </div>
        </div>
