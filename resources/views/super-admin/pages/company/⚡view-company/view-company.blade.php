        ...
        <div class="flex flex-col rounded-2xl border border-black/5 dark:border-white/5 bg-white dark:bg-[#242526] shadow-sm">
            ...
            <div class="px-6 py-5 grid gap-3 md:flex md:justify-between md:items-center border-b border-black/5 dark:border-white/5">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-xl font-bold text-[#123524] dark:text-white" style="font-family: 'Fraunces', serif;">All Companies</h2>
                        <p class="text-sm text-black/50 dark:text-white/50">Manage all companies</p>
                    </div>
                </div>
            ...
            @if(!empty($selectedCompanies))
                <div class="px-6 py-3 bg-red-50 dark:bg-red-500/10 border-b border-red-100 dark:border-red-500/20 flex items-center justify-between">
                    <p class="text-sm text-red-700 dark:text-red-400 font-medium">{{ count($selectedCompanies) }} company(ies) selected</p>
                    <button
                        x-data
                        @click="
                ...
                <table class="min-w-full text-xs sm:text-sm">
                        ...
                        <tr>
                            <th class="ps-6 py-3 w-4">
                                <input type="checkbox"
                                    ...
                                    class="rounded border-black/20 dark:border-white/20 text-[#123524] dark:text-[#D4A537] focus:ring-[#123524] dark:focus:ring-[#D4A537] bg-white dark:bg-[#3A3B3C]">
                            </th>
                            <th class="px-2 sm:px-6 py-3 text-start font-bold uppercase tracking-wide text-[#123524]/60 dark:text-white/60 text-[11px]">Company Name</th>
                            <th class="hidden sm:table-cell px-2 sm:px-6 py-3 text-start font-bold uppercase tracking-wide text-[#123524]/60 dark:text-white/60 text-[11px]">Address</th>
                    ...
                    <tbody class="divide-y divide-black/5 dark:divide-white/5">
                        @forelse ($this->companies as $company)
                            <tr class="hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors" wire:key="company-{{ $company->id }}">
                                <td class="w-4 ps-6 py-3 text-center align-middle">
                                    <input type="checkbox"
                                        ...
                                        class="rounded border-black/20 dark:border-white/20 text-[#123524] dark:text-[#D4A537] focus:ring-[#123524] dark:focus:ring-[#D4A537] bg-white dark:bg-[#3A3B3C] align-middle">
                                </td>
                                <td class="px-2 sm:px-6 py-3">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="font-semibold text-[#123524] dark:text-white">{{ $company->company_name }}</span>
                                    </div>
                                </td>
                                <td class="hidden sm:table-cell px-2 sm:px-6 py-3">
                                    <span class="text-black/70 dark:text-white/70">{{ $company->company_address ?? 'N/A' }}</span>
                                </td>
                                <td class="hidden md:table-cell px-2 sm:px-6 py-3">
                                    <span class="text-black/50 dark:text-white/50">{{ $company->created_at->diffForHumans() }}</span>
                                </td>
            ...
            <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-t border-black/5 dark:border-white/5">
                <p class="text-sm text-black/60 dark:text-white/60">
                    <span class="font-semibold text-[#123524] dark:text-[#D4A537]">{{ $this->companies->total() }}</span> results
                </p>
