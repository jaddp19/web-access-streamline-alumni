        ...
        <div class="flex flex-col rounded-2xl border border-black/5 dark:border-white/5 bg-white dark:bg-[#242526] shadow-sm">
            ...
            <div class="px-6 py-5 grid gap-3 md:flex md:justify-between md:items-center border-b border-black/5 dark:border-white/5">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-xl font-bold text-[#123524] dark:text-white" style="font-family: 'Fraunces', serif;">All Courses</h2>
                        <p class="text-sm text-black/50 dark:text-white/50">Manage all courses</p>
                    </div>
                </div>
            ...
            @if(!empty($selectedPrograms))
                <div class="px-6 py-3 bg-red-50 dark:bg-red-500/10 border-b border-red-100 dark:border-red-500/20 flex items-center justify-between">
                    <p class="text-sm text-red-700 dark:text-red-400 font-medium">{{ count($selectedPrograms) }} course(s) selected</p>
                    <button
                        x-data
                        @click="
                ...
                <table class="min-w-full text-xs sm:text-sm">
                    <thead class="bg-[#F7F5EF] dark:bg-[#3A3B3C] border-b border-black/5 dark:border-white/5">
                        <tr>
                            ...
                            <th class="px-2 sm:px-6 py-3 text-start font-bold uppercase tracking-wide text-[#123524]/60 dark:text-white/60 text-[11px]">Course Name</th>
                    ...
                    <tbody class="divide-y divide-black/5 dark:divide-white/5">
                        @forelse ($this->programs as $program)
                            <tr class="hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors" wire:key="program-{{ $program->id }}">
                                ...
                                <td class="px-2 sm:px-6 py-3">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="font-semibold text-[#123524] dark:text-white">{{ $program->course_title }}</span>
                                    </div>
                                </td>
                                <td class="hidden sm:table-cell px-2 sm:px-6 py-3">
                                    <span class="text-black/70 dark:text-white/70">{{ $program->department->dept_name ?? 'N/A' }}</span>
                                </td>
                                <td class="hidden md:table-cell px-2 sm:px-6 py-3">
                                    <span class="text-black/50 dark:text-white/50">{{ $program->created_at->diffForHumans() }}</span>
                                </td>
            ...
            <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-t border-black/5 dark:border-white/5">
                <p class="text-sm text-black/60 dark:text-white/60">
                    <span class="font-semibold text-[#123524] dark:text-[#D4A537]">{{ $this->programs->total() }}</span> results
                </p>
