<div>
            ...
            <div wire:loading.class="opacity-50 pointer-events-none" wire:target="selectedBatchId"
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-5">
                    ...
                    <div
                        ...
                        <div class="relative flex items-center gap-3">
                            <div
                                class="w-10 h-10 rounded-xl bg-[#1C6B45]/10 dark:bg-[#1C6B45]/20 flex items-center justify-center text-[#1C6B45] dark:text-emerald-400 shrink-0">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="1.5"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                    ...
                    <div
                        ...
                        @if (empty($this->courseAnalyticsByDept))
                            <p class="text-sm text-black/40 dark:text-white/40 py-16 text-center">No course analytics
                                yet.</p>
                        @else
                ...
                {{-- ===== Alumni by Year (DRILLABLE: batch → department → course) ===== --}}
                <div
                    class="bg-white dark:bg-[#242526] border border-black/5 dark:border-white/5 shadow-sm rounded-2xl p-4 md:p-5">
                    <div class="flex items-start justify-between gap-3 mb-3">
                        <div class="min-w-0">
                            ...
                            <p id="batch-subtitle" class="text-xs text-black/40 dark:text-white/40 mt-0.5">
                                Total graduates per batch · click a bar to see departments
                            </p>
                        </div>
                ...
                <div>
                    ...
                    <p class="text-sm text-black/50 dark:text-white/50 mt-0.5">Deeper breakdown from the tracer study.
                    </p>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                    <div
                        ...
                        <p class="text-xs text-black/40 dark:text-white/40 mb-3">Employed, unemployed, self-employed
                        </p>
                        @if (empty($this->employmentStatusBreakdown))
                            <p class="text-sm text-black/40 dark:text-white/40 py-16 text-center">No tracer employment
                                data yet.</p>
                        @else
                    ...
                    <div
                        ...
                        <p class="text-xs text-black/40 dark:text-white/40 mb-3">Full-time, part-time, freelance, etc.
                        </p>
                        @if (empty($this->employmentTypeBreakdown))
                            <p class="text-sm text-black/40 dark:text-white/40 py-16 text-center">No employment type
                                yet.</p>
                        @else
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                    <div
                        ...
                        @if (empty($this->organizationTypeBreakdown))
                            <p class="text-sm text-black/40 dark:text-white/40 py-16 text-center">No organization type
                                yet.</p>
                        @else
                    ...
                    <div
                        ...
                        @if (empty($this->employmentAreaBreakdown))
                            <p class="text-sm text-black/40 dark:text-white/40 py-16 text-center">No employment area
                                yet.</p>
                        @else
                ...
                <div
                    ...
                    @if (collect($this->monthsToFirstJobBreakdown)->sum() === 0)
                        <p class="text-sm text-black/40 dark:text-white/40 py-16 text-center">No time-to-first-job data
                            yet.</p>
                    @else
    ...
    <script type="application/json" id="analytics-payload">
        {!! json_encode([
            ...
            'alumniByBatchAndDepartment' => $this->alumniByBatchAndDepartment,
...
@script
    <script>
        ...
        const integerTicks = (dataMax = 0, baseTheme = null) => {
            const t = baseTheme || chartTheme();
            return {
                beginAtZero: true,
        ...
        const initCharts = async () => {
            ...
            const alumniByBatchDept = payload.alumniByBatchAndDepartment || {};
            ...
            if (Object.keys(typeData).length > 0) {
                ...
                makeChart(ChartLib, 'employmentTypeChart', {
                    ...
                    options: {
                        ...
                        scales: {
                            y: integerTicks(Math.max(0, ...typeValues), theme),
                            x: {
            ...
            if (Object.keys(monthsData).length > 0) {
                ...
                makeChart(ChartLib, 'monthsToFirstJobChart', {
                    ...
                    options: {
                        ...
                        scales: {
                            y: integerTicks(Math.max(0, ...monthValues), theme),
                            x: {
                                ticks: { color: theme.mutedText, font: { weight: '600' }, autoSkip: true, maxRotation: 45 },
                                grid: { display: false },
                                border: { display: false }
        ...
        const renderBatchChart = (ChartLib) => {
            ...
            if (state.view === 'batches') {
                ...
                totals = codes.map(c => batch.departments[c]?.total || 0);
                clickable = true;
                ...
                totals = codes.map(c => dept.courses[c]?.total || 0);
                clickable = false;
                subtitleText = `Batch ${batch.batch_name} · ${dept.name} · breakdown by course`;
                backBtnVisible = true;
            }
        ...
        if (!window.__analyticsBatchBack) {
            ...
            document.addEventListener('click', (e) => {
                ...
                if (drillState.batch.view === 'courses') {
                    drillState.batch.view = 'departments';
                    drillState.batch.selectedDept = null;
                } else {
