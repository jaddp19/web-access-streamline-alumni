            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                <div class="bg-white border border-black/5 shadow-sm rounded-2xl p-4 md:p-5">
                    ...
                    @if (empty($this->employmentStatusBreakdown))
                        <p class="text-sm text-black/40 py-16 text-center">No tracer employment data yet. Submit a tracer study first.</p>
                    @else
                ...
                <div class="bg-white border border-black/5 shadow-sm rounded-2xl p-4 md:p-5">
                    ...
                    @if (empty($this->employmentTypeBreakdown))
                        <p class="text-sm text-black/40 py-16 text-center">No employment type yet. Alumni must mark themselves as employed in the tracer form.</p>
                    @else
    ...
    waitForChart().then((ChartLib) => {
        ...
        makeChart(ChartLib, 'alumniDynamicChart', {
            ...
            data: {
                ...
                datasets: [{ data: Object.values(alumniByDept || {}), backgroundColor: '#16a34a', borderRadius: 6, maxBarThickness: 42 }]
            },
            options: {
                responsive: true, maintainAspectRatio: false,
                plugins: { legend: { display: false }, tooltip: { callbacks: { title: (i) => rawLabels[i[0]?.dataIndex] ?? '' } } },
        ...
        makeChart(ChartLib, 'comparativeChart', {
            ...
            data: {
                labels: compLabels,
                datasets: [
                    { label: 'Aligned with Work', data: (analyticsData || []).map((i) => i.related_rate), backgroundColor: '#16a34a', borderRadius: 6, maxBarThickness: 28 },
                    { label: 'Not Aligned', data: (analyticsData || []).map((i) => 100 - i.related_rate), backgroundColor: '#D4A537', borderRadius: 6, maxBarThickness: 28 }
                ]
            },
            options: {
                responsive: true, maintainAspectRatio: false,
                scales: { y: { beginAtZero: true, max: 100, grid: { color: '#f1f1f1' } }, x: { grid: { display: false } } },
        ...
        makeChart(ChartLib, 'employmentStatusChart', {
            ...
            data: {
                ...
                datasets: [{ data: Object.values(statusData || {}), backgroundColor: ['#16a34a', '#D4A537', '#3b82f6', '#94a3b8'], borderWidth: 2, borderColor: '#fff' }]
            },
        ...
        makeChart(ChartLib, 'employmentTypeChart', {
            type: 'bar',
            data: { labels: Object.keys(typeData || {}).map(pretty), datasets: [{ data: Object.values(typeData || {}), backgroundColor: '#16a34a', borderRadius: 6, maxBarThickness: 40 }] },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, grid: { color: '#f1f1f1' } }, x: { grid: { display: false } } } }
        });

        makeChart(ChartLib, 'organizationTypeChart', {
            type: 'bar',
            data: { labels: Object.keys(orgData || {}).map(pretty), datasets: [{ data: Object.values(orgData || {}), backgroundColor: '#D4A537', borderRadius: 6, maxBarThickness: 40 }] },
            options: { responsive: true, maintainAspectRatio: false, indexAxis: 'y', plugins: { legend: { display: false } }, scales: { x: { beginAtZero: true, grid: { color: '#f1f1f1' } }, y: { grid: { display: false } } } }
        });

        makeChart(ChartLib, 'employmentAreaChart', {
            type: 'pie',
            data: { labels: Object.keys(areaData || {}).map(pretty), datasets: [{ data: Object.values(areaData || {}), backgroundColor: ['#16a34a', '#3b82f6'], borderWidth: 2, borderColor: '#fff' }] },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } } }
        });

        makeChart(ChartLib, 'monthsToFirstJobChart', {
            type: 'bar',
            data: { labels: Object.keys(monthsData || {}).map(pretty), datasets: [{ data: Object.values(monthsData || {}), backgroundColor: '#3b82f6', borderRadius: 6, maxBarThickness: 45 }] },
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, grid: { color: '#f1f1f1' } }, x: { grid: { display: false } } } }
        });
    });
</script>
