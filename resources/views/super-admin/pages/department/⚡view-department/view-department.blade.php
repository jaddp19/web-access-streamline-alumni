        ...
        <div class="flex flex-col rounded-2xl border border-black/5 dark:border-white/5 bg-white dark:bg-[#242526] shadow-sm">
            ...
            <div class="px-6 py-5 grid gap-3 md:flex md:justify-between md:items-center border-b border-black/5 dark:border-white/5">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-xl font-bold text-[#123524] dark:text-white" style="font-family: 'Fraunces', serif;">All Departments</h2>
                        <p class="text-sm text-black/50 dark:text-white/50">Manage all departments</p>
                    </div>
                </div>
            ...
            @if(!empty($selectedDepartments))
                <div class="px-6 py-3 bg-red-50 dark:bg-red-500/10 border-b border-red-100 dark:border-red-500/20 flex items-center justify-between">
                    <p class="text-sm text-red-700 dark:text-red-400 font-medium">{{ count($selectedDepartments) }} department(s) selected</p>
                    <button
                        x-data
                        @click="
                ...
                <table class="min-w-full text-xs sm:text-sm">
                    <thead class="bg-[#F7F5EF] dark:bg-[#3A3B3C] border-b border-black/5 dark:border-white/5">
                        <tr>
                            ...
                            <th class="px-2 sm:px-6 py-3 text-start font-bold uppercase tracking-wide text-[#123524]/60 dark:text-white/60 text-[11px]">Name</th>
                            <th class="hidden sm:table-cell px-2 sm:px-6 py-3 text-start font-bold uppercase tracking-wide text-[#123524]/60 dark:text-white/60 text-[11px]">Code</th>
                            <th class="hidden sm:table-cell px-2 sm:px-6 py-3 text-start font-bold uppercase tracking-wide text-[#123524]/60 dark:text-white/60 text-[11px]">Status</th>
                            ...
                            <tr class="hover:bg-black/[0.02] dark:hover:bg-white/[0.03] transition-colors">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        @else
                                            <img src="{{ Storage::url($department->dept_logo) }}"
                                                ...
                                                class="w-8 h-8 rounded-full object-cover shrink-0">
                                        @endif
                                        <span class="font-semibold text-[#123524] dark:text-white">{{ $department->dept_name }}</span>
                                        @if($department->dept_desc)
                                        <span class="block text-xs text-black/50 dark:text-white/50 truncate max-w-[220px]">{{ $department->dept_desc }}</span>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="hidden sm:table-cell px-2 sm:px-6 py-3">
                                    @if ($department->is_active)
                                        <span class="inline-flex items-center text-[10px] px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 font-semibold uppercase tracking-wide">
                                            Active
                                        </span>
                                    @else
                                        <span class="inline-flex items-center text-[10px] px-2 py-0.5 rounded-full bg-black/5 dark:bg-white/10 text-black/40 dark:text-white/40 font-semibold uppercase tracking-wide">
                                            Inactive
                                        </span>
                                    @endif
                                </td>
                                <td class="hidden md:table-cell px-2 sm:px-6 py-3">
                                    <span class="text-black/50 dark:text-white/50">{{ $department->created_at->diffForHumans() }}</span>
                                </td>
            ...
            <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-t border-black/5 dark:border-white/5">
                <p class="text-sm text-black/60 dark:text-white/60">
                    <span class="font-semibold text-[#123524] dark:text-white">{{ $this->departments->total() }}</span> results
                </p>
