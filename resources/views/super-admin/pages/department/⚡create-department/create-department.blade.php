        ...
        <div class="max-w-2xl mx-auto">
            ...
            <div class="rounded-2xl border border-black/5 dark:border-white/5 bg-white dark:bg-[#242526] shadow-sm">
                ...
                <div class="px-6 py-5 flex items-center gap-4 border-b border-black/5 dark:border-white/5">
                    ...
                    <div>
                        <h2 class="text-xl font-bold text-[#123524] dark:text-white" style="font-family: 'Fraunces', serif;">Create Department</h2>
                        <p class="text-sm text-black/50 dark:text-white/50">Add a new department to the directory</p>
                    </div>
                </div>
                ...
                <form wire:submit.prevent="create" class="p-6 space-y-5">
                    ...
                    <!-- Name -->
                    <div>
                        <label for="dept_name" class="block text-xs text-black/60 dark:text-white/60 uppercase tracking-wide font-semibold mb-2">Department Name</label>
                        ...
                        @error('dept_name') <span class="text-red-500 dark:text-red-400 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <!-- Code -->
                    <div>
                        <label for="dept_code" class="block text-xs text-black/60 dark:text-white/60 uppercase tracking-wide font-semibold mb-2">Department Code</label>
                        <input wire:model.defer="dept_code" type="text" id="dept_code" placeholder="e.g. CS, BSIT"
                            class="w-full px-4 py-2.5 rounded-xl border border-black/10 dark:border-white/10 bg-[#F7F5EF] dark:bg-[#3A3B3C] text-black dark:text-white placeholder:text-black/40 dark:placeholder:text-white/40 focus:outline-none focus:border-[#123524] dark:focus:border-[#D4A537] focus:ring-1 focus:ring-[#123524] dark:focus:ring-[#D4A537] transition">
                        @error('dept_code') <span class="text-red-500 dark:text-red-400 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <!-- Description -->
                    <div>
                        <label for="dept_desc" class="block text-xs text-black/60 dark:text-white/60 uppercase tracking-wide font-semibold mb-2">Description</label>
                        ...
                        @error('dept_desc') <span class="text-red-500 dark:text-red-400 text-sm">{{ $message }}</span> @enderror
                    </div>

                    <!-- Logo -->
                    <div>
                        <label for="dept_logo" class="block text-xs text-black/60 dark:text-white/60 uppercase tracking-wide font-semibold mb-2">Department Logo</label>
                        ...
                        @if ($dept_logo)
                            <img src="{{ $dept_logo->temporaryUrl() }}" alt="Logo preview" class="mt-3 w-16 h-16 rounded-xl object-cover border border-black/10 dark:border-white/10">
                        @endif
                        @error('dept_logo') <span class="block text-red-500 dark:text-red-400 text-sm mt-1">{{ $message }}</span> @enderror
                    </div>

                    <!-- Active toggle -->
                    <div class="flex items-center justify-between bg-[#F7F5EF] dark:bg-[#3A3B3C] border border-black/10 dark:border-white/10 rounded-xl px-5 py-4">
                        <div class="pr-4">
                            <p class="font-semibold text-sm text-black dark:text-white">Active</p>
                            <p class="text-xs text-black/60 dark:text-white/60 mt-0.5">Inactive departments won't be selectable for new courses or alumni records.</p>
                        </div>
